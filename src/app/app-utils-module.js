    ...
    .factory("widget", ['$rootScope','lodash','user','$q','$http','$window', '$ocLazyLoad', function($rootScope, _, user,$q,$http,$window, $ocLazyLoad) {
      ...
      var api = {
            ...
            .then(function (list) { // wrap the loaded widget object in a promise
                ...
                if(widgetObj.javascripts !== undefined){
                    ...
                    for(var i = 0; i < widgetObj.javascripts.length; i++){
                        files.push(widgetObj.dir + widgetObj.javascripts);
                    }
        ...
        getTemplate:function(widgetName){
          var d = $q.defer();
          if(activeWidgets[widgetName] && activeWidgets[widgetName].template_html){
            d.resolve(activeWidgets[widgetName].template_html);
            return d.promise;
          }
          api.loadWidget(widgetName).then(function(widget){
              api.loadHtml(widget).then(function(htmlStr){
                  activeWidgets[widgetName].template_html = d.promise;
                  d.resolve(htmlStr);
              });
          });
          return d.promise;
