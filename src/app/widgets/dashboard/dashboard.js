define(['angularAMD', 'angular', 'jquery', 'angular-ui-bootstrap'], function(angularAMD, angular, $){
    var module = angular.module('realize-dashboard', ['realize-utils', 'ui.bootstrap'])

        .controller('DashboardCtrl', ['$scope', 'resource', 'dashboard', 'widget', '$rootElement', function($scope, resource, dashboard, widget, $rootElement){
            ...
            $scope.renderWidgets = function(){
              for(var i = 0; i < $scope.widgets.length; i++){
                  if($scope.widgets[i].rendered !== true){
                      ...
                      widget.addToPage(widgetData.name, $scope.widgetContainer.find('.' + widgetClass)).then(function(data){
